function [valid,J,n,calc_thrust,calc_power,calc_torque]=plane_propeller_conditions_given_torque_requirement(req_torque,speed,diam_m,pitch_m,Ct_load,Cp_load,eta_load,p_D_load,density,cruise_thrust)
J=0;
n=0;
calc_thrust=0;
calc_power=0;
...
while loop_count<50
    ...
    if(p_D_upper==p_D_lower)
         ...
         for i=1:length(p_D_load)
             ...
             if(p_D_load(i)==p_D_upper)
                 ...
                 Cq_est=Cp_est/(2*pi);
                 calc_torque=Cq_est*density*n*n*diam_m*diam_m*diam_m*diam_m*diam_m;
                 break;
             end
         end
    ...
    if(abs(calc_torque-req_torque)/req_torque<0.00001)
        break;
    else
        ...
        J=J+(calc_torque-req_torque)/req_torque*J*0.2;
    end
